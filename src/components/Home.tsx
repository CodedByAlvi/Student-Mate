...
export default function Home() {
  ...
  const stats = useMemo(() => [
    ...
    { label: 'To-Do List', count: pendingTasks.length, icon: CheckCircle2, color: 'text-brand-600', bg: 'bg-brand-50 dark:bg-brand-900/20', tab: 'tasks' },
    { label: 'Focus Score', count: focusScore, icon: Brain, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/20', tab: 'focus' },
        ...
        {stats.map((stat, index) => (
          ...
          >
            ...
            <div className="space-y-0.5 text-center">
              <span className="font-mono text-xl font-bold tracking-tighter leading-none block">
                {stat.count.toString().padStart(2, '0')}{stat.label === 'Focus Score' ? '%' : ''}
              </span>
          ...
          <AnimatePresence mode="wait">
            {upcomingExams.length > 0 && upcomingExams[currentExamIndex] ? (
              <motion.div
                ...
                className="relative overflow-hidden rounded-[2.5rem] border border-stone-200 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900"
              >
                {/* Background Accent */}
